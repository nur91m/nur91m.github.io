...
function updateStatisticsDisplay(stats) {
    document.getElementById('totalTasks').textContent = `Всего задач: ${stats.totalTasks}`;
    document.getElementById('correctAnswers').textContent = `Правильных: ${stats.correctAnswers}`;
    document.getElementById('incorrectAnswers').textContent = `Неправильных: ${stats.incorrectAnswers}`;
}
...
function displayTask(task) {
    document.getElementById("task").textContent = `${task.num1} x ${task.num2} = ?`;
...
function nextTask() {
    ...
    if (currentTaskIndex < tasks.length) {
        ...
        const stats = loadStatistics();
        document.getElementById("task-container").innerHTML = `<p>Тренировка завершена! Правильных ответов: ${stats.correctAnswers}/${stats.totalTasks}</p>`;
